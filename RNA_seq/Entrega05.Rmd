...
Luego leemos en una tabla `tx2gene` preconstruida:
...
El paquete `tximport` tiene una sola función para importar estimaciones a nivel de transcripción. 
...
```{r DESeqDataSetFromTximport, warning=FALSE}
...
# Extraer database homosapiens 
# dataset --> Especificar la especie 
# Host --> URL
HS_database = useMart(biomart = "ENSEMBL_MART_ENSEMBL", host = "https://ensembl.org", dataset = "hsapiens_gene_ensembl")

```

- Extraer genes de terminos GO
```{r}
# Importar libreria
library(topGO)

# Extraer genes de terminos GO
Hs_GOgenes <- getBM(c("ensembl_gene_id","go_id"), mart=HS_database)
head(Hs_GOgenes)
