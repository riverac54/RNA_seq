...
Con $padj < 0.05$, esperamos que $5 \%$ de los genes sean falsos positivosesto, obteniendo solo los genes mas significativos.

Tenemos mas de 9 genes que pertencen a la regulacion del desarrollo neuronal, su p.adjust es menor a 0.2, lo que indica su significancia, es decir, podemos afirmar que presenta esa funcion.

Asi mismo, contamos con menos de 6 genes que pertenecen a la 'negative regulation of response to wounding', la cual se encarga de disminuir la frecuencia de eventos en la restauracion de tejidos dañados posterior a una lesion. 
...
genes_up = rownames(data_up[data_up$log2FoldChange >= 0.5 & data_up$padj < 0.05, ])
...
# Convertirlo en matriz y utilizar solo los primeros 100 IDs del grupo UP
genes_down <- subset(Hs_res_all, padj < 0.01 & log2FoldChange < -2)
genes_down = genes_down[ order(genes_down$log2FoldChange, decreasing = FALSE), ] # Ordenar por log2FoldChange
genes_down = rownames(genes_down) # Nombres de los genes
genes_down = as.matrix(genes_down) # Hacer matriz
genes_down = genes_down[1:100,] # Tomar los primeros 100
...
```{r plot, eval= FALSE}
 # Generar la plot de los primeros 20 genes
plot(barplot(Go_results_UP, showCategory = 15, horiz=TRUE)) + barplot(Go_results_DOWN, showCategory = 15, horiz=TRUE) 
```

![](./Imagenes/Hs_groups.png)
