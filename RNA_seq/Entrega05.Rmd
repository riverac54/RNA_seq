...
Se puede encontrar el archivo reporte del analisis de MultiQC en el archivo `multiqc_report_rawData.html`.
</div>
...
<div class="alert alert-block alert-info">
Se puede encontrar el archivo reporte del analisis de MultiQC en el archivo `multiqc_report_trimmed.html`.
</div>
...
## Generar con subgrupos (analisis GO)
...
Finalmente, observamos que elementos de la familia de genes PSG (pregnancy-specific glycoprotein), como PSG11 y PSG4, mostraron un incremento en su expresión durante la diferenciación de syncytiotrophoblasts (SynT), como es reportado por Kwak et al., 2019. 

<div class="alert alert-block alert-info">
**NOTA**: Para poder identificar la sobreexpresión del gen *NOS3* (y los demás mencionados), se realizó lo siguiente : __(1)__ Identificar el nombre del gen dentro del paper. __(2)__ En terminal, buscar el id del gen dentro del archivo de anotaciones [(`gencode.v43.annotation.gtf`)](https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_43/) con el comando `cat gencode.v43.annotation.gtf | grep 'ERVFRD' | head`. __(3)__ Buscar si el gen está sobre o subregulado en nuestros datos con `Hs_de["ENSG00000244476.3",]` (dentro de la consola de Rstudio).
</div>


En el articulo “*Endogenous retrovirus-derived enhancers confer the transcriptional regulation of human trophoblast syncytialization *” se utilizan marcadores de histonas para comparar entre los elementos regulatorios que son compartidos o especificos en hTSC y STB, esto con el objetivo de estudiar la funcion de los ERVs en la actividad regulatoria de genes y la generacion de trofoblastos.

Para estudiar esto primero se identificaron los genes que estan activos en STB (syncytiotrophoblast). Después vieron la ocurrencia de aparicion de ERVs cerca de los genes previamente identificados. Entre los ERVs se identificaron identifico a MER41A, MER50 and LTR8B. Se encontro que “MER50” esta mas presente entre los enhancers y los genes en STB. 

Para estudiar como afecta a la celula utilizaron un gen reportero “luciferase” con y sin su promotor para medir la actividad de MER50 como promotor.Se encontro una relacion entre MER50 y los genes involucrados en la formacion de STB. 

Genes como ERVFRD-1, MFSD2A, SPHK1, PSGs, IFITMs y ERVV-2  pertenecen a MER50. estos genes se encontraban presenten entre los datos que habian generados en la expresion diferencial, algunos como 'ERVFRD-1' no se encontraban expresados, esto tiene sentido, puesto que solo aparecen en las celulas ya diferenciadas (BTS).
...
- Kwak, Y.-T., Muralimanoharan, S., Gogate, A. A., & Mendelson, C. R. (2019). Human Trophoblast Differentiation Is Associated With Profound Gene Regulatory and Epigenetic Changes. Endocrinology, 160(9), 2189–2203. https://doi.org/10.1210/en.2019-00144

- Miao Yu, Xiaoqian Hu, Zihang Pan, Cui Du, Jing Jiang, Wanshan Zheng, Han Cai, Yinan Wang, Wenbo Deng, Haibin Wang, Jinhua Lu, Ming-an Sun, Bin Cao, Endogenous retrovirus-derived enhancers confer the transcriptional regulation of human trophoblast syncytialization, Nucleic Acids Research, 2023;, gkad109, https://doi.org/10.1093/nar/gkad109